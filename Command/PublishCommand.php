...
class PublishCommand extends Command
{
    ...
    {
        $this->setName('pumukit:timed:pub:decisions:publish:objects')
            ->setDescription(
...
The <info>pumukit:timed:pub:decisions:publish:objects</info> publish multimedia objects with a temporal decision.

To use in a crontab:

  <info>crontab:*/5 * * * * apache /usr/bin/php /var/www/pumukit/bin/console pumukit:timed:pub:decisions:publish:objects</info>
...
EOT
    ...
    protected function updateMultimediaObjects(OutputInterface $output, $timedCode)
    {
        ...
        if (0 !== $mms->count()) {
            ...
            $output->writeln('No multimedia objects to update');
        }
